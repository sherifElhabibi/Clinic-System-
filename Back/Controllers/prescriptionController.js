...
exports.addPrescription = async (request, response, next) => {
  let newPrescription = await new prescriptionSchema({
    ...
    prescriptionDate: `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()}`,
  });
  newPrescription
    .save()
    .then((result) => {
...
exports.deletePrescription = (request, response, next) => {
  prescriptionSchema
    ...
    .then((result) => {
      response.status(201).json({ result, message: "Prescription has been deleted" });
    })
...
exports.getPrescriptionByDoctorId = (request, response, next) => {
  prescriptionSchema
    ...
    .then((result) => {
      if (result != null) {
        ...
        next(new Error("Prescription doesn't exist for this doctor"));
      }
    })
    .catch((error) => {
      next(error);
